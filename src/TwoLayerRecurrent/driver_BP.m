% Trains a network for every output feature using backpropagation with an
% adaptive learning rate.  Saves the weight matrices and error vectors from
...
function driver_BP(mode)
    ...
    test_errors = zeros(num_features, 1);

    %train a network for every feature
    for i=1:num_features
        disp(i);
        [Winput, Winterior, Wprev1, Wprev2, Woutput, train_error, validate_error, test_error] = train_BP(datas(i), Winput_init, Winterior_init, Wprev1_init, Wprev2_init, Woutput_init, batch_size, batches);
        Winputs(:,:,i) = Winput;
    ...
    %save all the data
    save('BP.mat', 'test_errors', 'train_errors', 'validate_errors', 'datas', 'Winputs', 'Winteriors', 'Wprev1s', 'Wprev2s', 'Woutputs', 'std_mean');
end
