function error = trainingErrorChecker2L(data, std_mean)
    ...
    for j=1:6
        [temp_y, ~, ~,~, ~] = feedForward_new([X ones(size(X,1), 1)], Winput, Winterior, Wprev1, Wprev2, Woutput);
        ...
        [naive_y, ~, ~] = feedForward_new([X_naive ones(size(X_naive,1), 1)], Winput_init, Winterior_init, Wprev1_init, Wprev2_init, Woutput_init);
    ...
    save('post_batch_train.mat', 'Winput', 'Wprev1', 'Wprev2', 'Winterior', 'Woutput', 'data', 'std_mean', 'error');
