...
function checker_BP_small(data, std_mean)
    ...
    Winput_init = initWeights(7, num_neurons,-1/10, 1/10); % Create a d + 1 x n matrix for the extra bias feature
    ...
    for j=1:6
        ...
        values_actual(j,:) = (values_actual(j,:) .* std_mean(1,2)) + std_mean(2,2); %Restore the actual values too
        values_full = data(i+num_stacks-1+j,:);
        ...
        naive_full = data(i+num_stacks-1+j,:);   
        naive_full(1,2) = naive_y(end,:);
        X_naive = [X_naive(2:size(X_naive,1), :) ; naive_full];
    end
